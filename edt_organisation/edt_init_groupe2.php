...
<div id="lecorps">
  ...
  while( $exportLine = fgetcsv($exportFile, 1024, ";")  ){
    ...
    if($nbreGrp > 1){
      ...
      $listres = mysql_query($listreq) OR DIE('erreur dans la requête '.$listreq.' : '.mysql_error());
      ...
      while( $coursTab = mysql_fetch_array($listres) ){
        ...
        while($coursStartPer < $coursEndPer){
          ...
          $resolvRes = mysql_query($resolvReq) OR DIE('erreur dans la requête '.$resolvReq.' : '.mysql_error());
          $resolvn = mysql_num_rows($sqlres);
          
          if(mysql_num_rows($resolvRes) == 1){
            $coursResolvFlag = 1;
            break;
          }
      ...
      $sqltab = mysql_fetch_array($sqlres);
