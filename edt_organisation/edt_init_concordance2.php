...
if ($etape != NULL) {
	...
	if ($etape != 12) {
		...
		while( $sqltab = mysqli_fetch_array($sqlres) ){
		  ...
		  $fusionres = mysqli_query($GLOBALS["mysqli"], $fusionreq) OR DIE('erreur dans la requête '.$fusionreq.' : '.mysqli_error($GLOBALS["mysqli"]));

		  //on efface le premier cours
		  $deletereq = "DELETE FROM edt_cours WHERE id_cours = '".$fusion_delete."' ;";

		  $deleteres = mysqli_query($GLOBALS["mysqli"], $deletereq) OR DIE('erreur dans la requête '.$deletereq.' : '.mysqli_error($GLOBALS["mysqli"]));
		}
